...
writeStates <- function(model.path = NULL, layer.depth = NULL, layer_name = NULL, sequence = NULL, fasta.path = NULL, round_digits = 2,
  ...
  if (!file_output) {
    if (!returnStates) stop("If filename is NULL, returnStates must be TRUE; otherwise function produces no output.")
    filename <- tempfile(fileext = paste0(".", file_type))
  }
  stopifnot(batch.size > 0)
  ...
  if (!is.null(sequence) && (!missing(sequence) & sequence != "")) {
    ...
    if (nrow(fasta.file) > 1) {
      m_text <- paste("Your file has", nrow(fasta.file), "entries. writeStates will concatenate them to a single sequence.
                       Use writeStatesByFastaEntries to evaluate entries separately.")
      message()
    }
