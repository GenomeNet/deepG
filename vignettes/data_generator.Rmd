...
The most common use case for the deepG data generator is to extact samples from a collection of 
...
+ **Language model**: Part of a sequence is the input and other part the target.
    + Example: Predict the next nucleotide given the previous 100 nuleotides. 
...
If we want to extact sequences of length 4 from these files, there would be 17 possible samples 
(5 from <tt>AACCAAGG</tt>, 3 from <tt>TTTGGG</tt>, ...). 
A naive approach would be to go sequentially through all possible samples: 
...
Choosing the samples completely at random can also be problematic since we would constantly have to open new fasta files.
...
Possible arguments for <u> *language model* </u> are:
...
nuleotide sequence could be one of the labels "bacteria" or "virus". We have to specify how to extract a label corresponding to a sequence. 
...
column). Example: if we are currently working with a file called "a.fasta", there should be a row in our csv file <br>

<center>
<span>
file  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  | label_1 | label_2 <br>
"a.fasta" &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
1     &nbsp; &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp;      0 <br>
</span>
</center>
...
**"target_middel_cnn"**: $X =$ <tt>abcefg</tt>, $Y =$ <tt>d</tt> 
...
We may determine how frequently we want to take a sample. If `step = 1` we take a sample at every possible step. For the example, if `maxlen = 3, step = 1`. 
...
A sequence might contain a character that does not lie inside our desired vocabulary. For example, let's assume we discard <tt>e</tt> from our vocabulary.
...
If you use `max_samples`, generator will randomly choose subset from all possible sampels, but those samples are consecutive. With `random_sampling = TRUE`,
...
Target length for language model. Default is 1; if > 1 the target part of the generator output is a list of length `target_len`.
...
y3 <- z[[2]][[3]]
colnames(x) <- vocabulary
colnames(y1) <- vocabulary
colnames(y2) <- vocabulary
colnames(y3) <- vocabulary
x # abcde
y1 # f
y2 # g
y3 # h
...
List contains arguments \code{noise_type}: either "normal" or "uniform" and
...
the generator will randomly use 3 of the entries.

### shuffle_file_order 

Shuffle file order before iterating through files. Order gets reshuffles after every iteration.
...
+  `reshape_mode`: `"time_dist", "multi_input"` or `"concat"`. 
     + If `reshape_mode` is "multi_input", generator will produce `samples_per_target` separate inputs, each of length maxlen. 
     + If `reshape_mode` is "time_dist", generator will produce a 4D input array. The dimensions correspond to
...
+   If `reshape_mode = "concat"`, there is an additional `buffer_len` argmument: add new token between 
   concatenated samples
    + If `buffer_len` is an integer, the subsequences are interspaced with `buffer_len` rows. The input length is
...
```{r warning = FALSE, message = FALSE}
gen <-  get_generator(path = file.path(dir_path, "train_files_1"),
                      train_type = "lm", 
                      batch_size = 1,
                      maxlen = 5,
                      output_format = "target_right",
